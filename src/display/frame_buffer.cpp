...
Texture FrameBuffer::getTargetAsTexture(Target target) const
{
  ...
  PBL_ASSERT(!tex.empty(), "Tried to retrieve a non-existing texture");
  return tex;
}

FrameBuffer::~FrameBuffer()
{
  PBL_ASSERT(std::ranges::find(s_frameBuffersStack, this) == s_frameBuffersStack.end(), "Cannot delete a bound framebuffer");
...
void FrameBuffer::bind()
{
  ...
  PBL_ASSERT(s_frameBuffersStack.size() < 10, "Too many framebuffers on the stack! Did you forget to unbind() some ?");
}

void FrameBuffer::unbind()
{
  if (m_renderTargetView == nullptr && m_depthStencilView == nullptr) return;
  PBL_ASSERT(!s_frameBuffersStack.empty() && s_frameBuffersStack.back() == this, "Tried to unbind a framebuffer that was not bound!");
...
void FrameBuffer::bindToD3DContext() const
{
  ...
  WindowsEngine::d3dcontext().OMSetRenderTargets(1, &m_renderTargetView, m_depthStencilView);
  WindowsEngine::d3dcontext().RSSetViewports(1, &viewport);
}
...
{
  PBL_ASSERT(pastFrameCount < SAVED_FRAMES-1, "Invalid pastFrameCount");
...
{
  WindowsEngine::d3dcontext().IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
  WindowsEngine::d3dcontext().Draw(3, 0);
}
